...
async def list_threads(db: AsyncSession = Depends(get_db)):
    # TODO: Get user_id from auth token. For now use default user_id=1
    user_id = 1 
...
async def update_thread(thread_id: str, thread_update: ThreadUpdate, db: AsyncSession = Depends(get_db)):
    user_id = 1
    chat_service = ChatService(db)
